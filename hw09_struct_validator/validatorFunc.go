...
var arrayFunc = map[string]validStruct{
	"len":    validStruct{"len", validateLen, errors.New("Значение превысило максимальную длину")},
	"regexp": validStruct{"regexp", validateRegExp, errors.New("Значение не соответствует регулярному выражению")},
	"min":    validStruct{name: "min", fun: validateMinMax, err: errors.New("Значение меньше минимального")},
	"max":    validStruct{name: "max", fun: validateMinMax, err: errors.New("Значение больше максимального")},
	"in":     validStruct{name: "in", fun: validateIn, err: errors.New("Значение не входит в перечень")},
}
